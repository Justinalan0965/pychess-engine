...
class Chess:
    ...
    def arrage_pieces(self):
        # White pieces
        self.board[0][0] = self.helper.create_and_get_piece('rook', True, 0, 0)
        ...
        self.board[0][3] = self.helper.create_and_get_piece(
            'queen', True, 0, 3)
        self.board[0][4] = self.helper.create_and_get_piece('king', True, 0, 4)
        self.board[0][5] = self.helper.create_and_get_piece(
            'bishop', True, 0, 5)
        self.board[0][6] = self.helper.create_and_get_piece(
            'knight', True, 0, 6)
        self.board[0][7] = self.helper.create_and_get_piece('rook', True, 0, 7)
        for i in range(8):
    ...
    def move(self, piece, new_pos):
        ...
        if (self.isWhiteMove and piece.isWhite) or (not self.isWhiteMove and not piece.isWhite):
            ...
            self.move_history["white" if not self.isWhiteMove else "black"].append(
                (FROM, TO))
    ...
    def display_board(self):
        ...
        largest_piece_length = max(len(piece.symbol)
        ...
        for i in range(7, -1, -1):
            ...
            for j in range(8):
                ...
                print(piece.symbol.center(largest_piece_length), end="  ")
            print()

        print("  ", end="")
        for letter in alpbt_pos:
            print("[blue]" + letter.center(largest_piece_length), end="  ")
        print()
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            crnt_pos = chess.helper.convert_algebraic_to_indices(
                selected_piece)
            print(
                f"[green]You selected: [blue]{selected_piece} and its current position is: {crnt_pos}")
            ...
            if not available_moves:
                print(
                    "[red]No available moves for the selected piece. Please select another piece.")
                continue

            print("[green]Available moves for the selected piece [{} ]:".format(
                piece.symbol))
            print(
                f"[blue] {[chess.helper.get_algebraic_pos(move[0], move[1]) for move in available_moves]}")
